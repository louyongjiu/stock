...
def get_indicators(data, end_date=None, threshold=120, calc_threshold=None):
    try:
        ...
        logging.error(f"calculate_indicator.get_indicators处理异常：{data['code']}代码{e}", exc_info=True)
    return None
...
def get_indicator(code_name, data, stock_column, date=None, calc_threshold=90):
    try:
        ...
        logging.error(f"calculate_indicator.get_indicator处理异常：{code}代码{e}", exc_info=True)
    return None
